#' Indicator: Taxa de internação hospitalar (SUS) por condições sensíveis à atenção primária : Asma
#'
...
#' # Some examples
#' \dontrun{
#' indi_0038(agg = "mun_res", ano = 2013)
#' }
#'
...
indi_0038 <- function(
  ...
  Q1 <- glue::glue_collapse(
    sQuote(rpcdas::cid_seq("J450", "J451"), q = FALSE),
    sep = ", "
  )
  Q2 <- glue::glue_collapse(
    sQuote(rpcdas::cid_seq("J458", "J459"), q = FALSE),
    sep = ", "
  )
