#' Indicator: Taxa de internação hospitalar (SUS) por condições sensíveis à atenção primária : Bronquite
#'
...
#' # Some examples
#' \dontrun{
#' indi_0040(agg = "mun_res", ano = 2013)
#' }
#'
...
indi_0040 <- function(
  ...
  Q1 <- glue::glue_collapse(
    sQuote(rpcdas::cid_seq("J200", "J209"), q = FALSE),
    sep = ", "
  )
  Q2 <- glue::glue_collapse(
    sQuote(rpcdas::cid_seq("J219", "J459"), q = FALSE),
    sep = ", "
  )

  filter_query <- glue::glue("LEFT(DIAG_PRINC, 3) IN ({Q1}, {Q2})")
