...
indicator_adjusted <- function(numerador, nome, agg, multi, decimals){
  res2 <- mapply(cbind, numerador, "age_group" = age_groups_names, SIMPLIFY = FALSE) %>%
    dplyr::bind_rows() %>%
    tibble::as_tibble() %>%
    ...
    dplyr::group_by(agg, ano, age_group) %>%
    dplyr::summarise(freq = sum(freq, na.rm = TRUE), .groups = "drop") %>%
    dplyr::ungroup()

  # Standart population
  stdpop <- brpop::mun_pop() %>%
  ...
  res3 <- dplyr::inner_join(res2, brpop::mun_pop() %>% dplyr::filter(year == 2010), by = c("agg" = "mun", "ano" = "year", "age_group" = "age_group")) %>%
  ...
  res5 <- furrr::future_map_dfr(.x = res4, .f = function(x){
    c(id = x$agg[1], epitools::ageadjust.direct(count = x$count, pop = x$pop, stdpop = x$stdpop))
  })
